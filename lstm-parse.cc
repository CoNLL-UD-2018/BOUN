...
vector<unsigned> log_prob_parser(ComputationGraph* hg,
  ...
  //cerr << endl;
    ...
    for (unsigned i = 0; i < sent.size(); ++i) {
     ...
     /* unique_ptr<model> model(model::load("home/betul/udpipe/udpipe-models-2017/models/turkish-ud-2.0-conll17-170315.udpipe"));

      pipeline pline(model.get(), "generic_tokenizer", pipeline::DEFAULT, pipeline::DEFAULT, "conllu");

      std::string error;

      istringstream str("Ben eve gittim."); 
      std::istream A(str.rdbuf());   // shares the same buffer device with iss


      //std::unique_ptr<std::istream> stream;
      //stream = std::make_unique<std::istringstream>(std::istringstream{});
      if(!pline.process(A,cout,error))
      ...
      if (USE_SPELLING) {
		...
		strevbuffer.pop_back();
            ...
            /*for (unsigned j=w.length()-1;j>0;j=j-UTF8Len(w[j])) {
                ...
                //cerr<<j<<":"<<w[j]<<"\n"; 
                std::string wj;
                for (unsigned h=j;h<j+UTF8Len(w[j]);h++) wj+=w[h];f
                int wjint=corpus.charsToInt[wj];
                Expression cj=lookup(*hg, char_emb, wjint);
                bw_char_lstm.add_input(cj);
...
int main(int argc, char** argv) {
  ...
  if (true) { // do test evaluation
    ...
    for (unsigned sii = 0; sii < corpus_size; ++sii) {
      ...
      output_conll(sentence, lemmaSentTest, morpSentTest, sentencePos, sentenceUnkStr, corpus.intToWords, corpus.intToPos, hyp, rel_hyp);
      correct_heads += compute_correct(ref, hyp, sentence.size() - 1);
      total_heads += sentence.size() - 1;
    }
    auto t_end = std::chrono::high_resolution_clock::now();
