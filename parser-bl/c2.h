    ...
    ReplaceStringInPlace(lineS, "-LRB-", "_LRB_");
      ...
      current_sent.clear();
        ...
        do {
          ...
          //mycode: if a word contains space char, it fails. this code is aimed to fix this problem
         ...
         /* std::cerr << word << std::endl;
          size_t underScoreInd = word.rfind('-');
          if(underScoreInd != std::string::npos)
              std::cerr << "cant find underscore in '" << word << "'" << std::endl;

          assert(underScoreInd != std::string::npos);
         ...
         // std::cerr << "word: " << word << " " << pos <<  std::endl;
        // std::cerr << file.substr(file.find('_')+1) << std::endl;
          if(file.substr(file.find('_')+1) == "cs-pdt.txt" || 
             file.substr(file.find('_')+1) == "fi-ftb.txt" || 
             file.substr(file.find('_')+1) == "bg-btb.txt" || 
             file.substr(file.find('_')+1) == "cs-fictree.txt" ||
             file.substr(file.find('_')+1) == "grc-perseus.txt" ||
             file.substr(file.find('_')+1) == "lv-lvtb.txt" ||
             file.substr(file.find('_')+1) == "pl-lfg.txt" ||
             file.substr(file.find('_')+1) == "pl-sz.txt" ||
             file.substr(file.find('_')+1) == "sk-snk.txt" ||
             file.substr(file.find('_')+1) == "sl-ssj.txt" ||
             file.substr(file.find('_')+1) == "uk-iu.txt" ||
             file.substr(file.find('_')+1) == "sl-sst.txt" ||
             file.substr(file.find('_')+1) == "en-lines.txt" ||
             file.substr(file.find('_')+1) == "ga-idt.txt" ||
             file.substr(file.find('_')+1) == "la-ittb.txt" ||
             file.substr(file.find('_')+1) == "la-perseus.txt" ||
             file.substr(file.find('_')+1) == "sv-lines.txt" ||
             file.substr(file.find('_')+1) == "mix-lang.txt" ||
             file.substr(file.find('_')+1) == "ko-gsd.txt" ||
             file.substr(file.find('_')+1) == "ko-kaist.txt" ) //upos_xpos da problem çýkan datasetler için sadece upos almaca 
          { 
             if (pos.find('_') != std::string::npos)
             {
                // std::cerr << "found: " << pos << std::endl;
                 pos = pos.substr(0, pos.find('_'));
                 // std::cerr << "now: " << pos << std::endl;
          ...
          if (posToInt[pos] == 0) {
            posToInt[pos] = maxPos;
            intToPos[maxPos] = pos;
           // std::cerr << intToPos[maxPos] << "#" <<  pos << std::endl;
...
inline void load_correct_actionsDev(std::string file) {
  ...
  while (getline(actionsFile, lineS)) {
    ...
    if (lineS.empty()) {
     ...
     // std::cerr << "current_sent: " << std::endl;
    //  std::cerr << current_sent_str << std::endl;
      sentence++;
      ...
      if (initial) {
        ...
        do {
          ...
          //mycode
          /*size_t underScoreInd = word.rfind('_');
          if(0underScoreInd != std::string::npos)
              std::cerr << "cant find the underscore in '" << word << "'" << std::endl;
