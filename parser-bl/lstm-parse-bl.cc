...
//mycode
vector<string> tokenizeMorpFeats(const string& morps, const string& langCode)
{
     ...
     string varyingKeyword = "";
     string varyingKeyword2 = "";
     string varyingKeyword3 = "";

     if(langCode == "cu-proiel.txt" || langCode == "de-gsd.txt" || langCode == "en-ewt.txt" || langCode == "en-gum.txt" ||
        langCode == "en-lines.txt" || langCode == "es-ancora.txt" || langCode == "et-edt.txt" || langCode == "fi-ftb.txt" ||
        langCode == "fi-tdt.txt" || langCode == "ga-idt.txt" || langCode == "gl-treegal.txt" || langCode == "got-proiel.txt" ||
        langCode == "hr-set.txt" || langCode == "hsb-ufal.txt" || langCode == "hu-szeged.txt" || langCode == "kmr-mg.txt" ||
        langCode == "la-ittb.txt"  || langCode == "la-perseus.txt" || langCode == "la-proiel.txt" || langCode == "ro-rrt.txt" ||
        langCode == "sme-giella.txt" || langCode == "ur-udtb.txt" || langCode == "ca-ancora.txt" || langCode == "fr-gsd.txt" ||
        langCode == "fr-sequoia.txt" || langCode == "fr-spoken.txt" || langCode == "it-isdt.txt" || langCode == "it-postwita.txt" ||
        langCode == "pt-bosque.txt" || langCode == "sr-set.txt")
     {
          varyingKeyword = "Mood";
     }
     else if(langCode == "he-htb.txt")
           varyingKeyword = "HebBinYan";
     else if(langCode == "nl-alpino.txt" || langCode == "nl-lassysmall.txt") 
           varyingKeyword = "Degree";
     else
          varyingKeyword = "Aspect";

     if(langCode == "ar-padt.txt")
           varyingKeyword2 = "Mood";
     else
           varyingKeyword2 = "Tense";

     if(langCode == "ca-ancora.txt")
           varyingKeyword3 = "AdpType";
     
     else if(langCode == "he-htb.txt")
           varyingKeyword3 = "HebSource";
     
     else if(langCode == "it-isdt.txt" || langCode == "it-postwita.txt" || langCode == "pt-bosque.txt" || langCode == "sr-set.txt")
           varyingKeyword3 = "PronType";

     else
           varyingKeyword3 = "Case";
     ...
     for (const auto& t : tokens) {
         if(boost::starts_with(t, varyingKeyword3))
         {
            std::size_t pos = t.find("=");

            std::string featValue = t.substr(pos+1); 
            mcase = featValue;
            if(langCode == "fr-gsd.txt" || langCode == "fr-sequoia.txt" || langCode == "fr-spoken.txt" || langCode == "fro-srcmf.txt")
               mcase = "";
         }
         else if(boost::starts_with(t, varyingKeyword2))
         {
            std::size_t pos = t.find("=");
         ...
         else if(boost::starts_with(t, varyingKeyword))
         {
            std::size_t pos = t.find("=");

            std::string featValue = t.substr(pos+1); 
            aspect = featValue;
            if(langCode == "fro-srcmf.txt")
               aspect = "";
         }
     }
...
*/
    ...
    for (unsigned i = 0; i < sent.size(); ++i) {
      ...
      if(lemmaSentTest.at(0) == "###?###")
      {
         ...
         {
           //cerr << "lemma: "<< lemmaSent.at(i).suffixes.at(j) << endl;
  ...
  //    vector<string> morpFeatures;
   //   morpFeatures = tokenizeMorpFeats(morpSentTest.at(i), langCode);
		...
		*/											                                  
	    ...
	    Expression fw_a = createLstmCharEmbForward(ww_aspect, fw_char_lstm_aspect, hg, word_start, word_end, strevbuffer6);
          ...
          */      
             ...
             vector<Expression> tt = {fw_r, bw_r};   //, fw_s, bw_s}; // fw_t, fw_vf, fw_a};
...
int main(int argc, char** argv) {
  ...
  std::string traOracleName = conf["training_data"].as<string>();
  std::string langCode = traOracleName.substr(traOracleName.find('_')+1);
   //mycode
    ...
    while(!requested_stop) {
      ...
      for (unsigned sii = 0; sii < status_every_i_iterations; ++sii) {
	   ...
	   const vector<unsigned>& actions=corpus.correct_act_sent[order[si]];
           ComputationGraph hg;
           parser.log_prob_parser(&hg,sentence,tsentence, langCode, lemmaSentDummyTrain, lemmaSentTrain, morpSentTrain, sentencePos,actions,corpus.actions,corpus.intToWords,&right);
      ...
      if (logc % 25 == 1) { // report on dev set
	   ...
	   if (!USE_SPELLING) {
           ...
           ComputationGraph hg;
	   vector<unsigned> pred = parser.log_prob_parser(&hg,sentence,tsentence, langCode, lemmaSentDummyDev, lemmaSentDev, morpSentDev, sentencePos,vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
	   double lp = 0;
  ...
  if (true) { // do test evaluation
    ...
    for (unsigned sii = 0; sii < corpus_size; ++sii) {
     ...
     /* for(unsigned ff=0; ff< lemmaSentTest.size(); ff++)
      {
          
           cerr << lemmaSentTest.at(ff) << " " << morpSentTest.at(ff) << " " << uposSentTest.at(ff) << " " << xposSentTest.at(ff) << " " << spaceSentTest.at(ff) <<  endl;
      }*/ 
      ...
      if (beam_size == 1)
        pred = parser.log_prob_parser(&cg,sentence,tsentence, langCode, lemmaSentDummyTest, lemmaSentTest, morpSentTest, sentencePos,vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
      else
