...
//      bw_char_lstm(LAYERS, LSTM_CHAR_OUTPUT_DIM, LSTM_INPUT_DIM,  model), //Miguel
      ...
      bw_char_lstm_stem(LAYERS, LSTM_INPUT_DIM, LSTM_CHAR_OUTPUT_DIM/2, model)/*mycode ends*/ {
    if (USE_POS) {
...
vector<unsigned> log_prob_parser(ComputationGraph* hg,
 ...
 /*   //my code begins -- cümlenin lemmasýný bulma

    //sentence ý oluþturma
    ...
    for (unsigned i = 0; i < sent.size()-1; ++i) {
        assert(sent[i] < VOCAB_SIZE);
         ...
         unsigned wid=sent[i];
         std::string ww=intToWords.at(wid);
    ...
    //cout << den << endl;
    vector<string> lemma_sent;
    std::string command = "";
    command.append("curl -F data='");
    command.append(den);
    command.append("' -F model=turkish -F tokenizer= -F tagger= http://lindat.mff.cuni.cz/services/udpipe/api/process 2>&1");

    std::string result = exec(command.c_str());
   // cout << "lets start again"  << endl;
    std::string delimiter = "\t";
    size_t pos = 0;
    std::string token;
    int ctr = 0;
    while ((pos = result.find("\\t")) != std::string::npos) {
          token = result.substr(1, pos-1);
                         
          if(ctr%9 == 2)
          {
             // cout << " " << token ;
              lemma_sent.push_back(token);
                             
          }
          ctr++;    
          result.erase(0, pos + delimiter.length());
    }
    lemma_sent.push_back("ROOT");
  
                   
    //my code ends  */
    //my code starts
  //  cout << sent.size() << "sent - lem" << lemma_sent.size() << endl;
    ...
    for (unsigned i = 0; i < sent.size(); ++i) {
      ...
      std::string lemma = lemmaSent.at(i).lemma;

     // cout << ww << " " << lemmaSent.at(i).word << " " << lemmaSent.at(i).lemma << endl;
/*   //my code starts
      std::string ww2 = boost::algorithm::to_lower_copy(ww);
      std::string lemma = "";
      if(sent.size() == lemma_sent.size())
         lemma = lemma_sent.at(i);
      else
         lemma = ww2;
    //  cout << ww2 << " " << lemma << endl;
*/
     ...
     // cout << utf8_strlen(ww2) << " " << utf8_strlen(lemma) << " " << stem_length << endl;
        ...
        if (ww.length()==4  && ww[0]=='R' && ww[1]=='O' && ww[2]=='O' && ww[3]=='T'){
          w=lookup(*hg, p_w, sent[i]); //we do not need a LSTM encoding for the root word, so we put it directly-.
		    ...
		    //mycode starts
				...
				std::string ww_root = lemma; //utf8_substr(ww,0,ww.length()-3);
				std::string ww_stem = stem; //utf8_substr(ww,ww.length()-3,ww.length()-1);
                                //std::cout << ww << " length: " << ww.length() << endl;
				//root of a word
				fw_char_lstm_root.start_new_sequence();
				fw_char_lstm_root.add_input(word_start);
				std::vector<int> strevbuffer;
                              
				for (unsigned j=0;j<ww_root.length();j+=UTF8Len(ww_root[j])){  
                                      // std::cout << j << " " << ww_root << " root char len: " << ww_root.length() <<" " << ww_root[j] << UTF8Len(ww_root[j]) << endl;
			            std::string wj;
                                    for (unsigned h=j;h<j+UTF8Len(ww_root[j]);h++) wj+=ww_root[h];
               
                                    int wjint=corpus.charsToInt[wj];
		 
		                    strevbuffer.push_back(wjint);
                                    Expression cj=lookup(*hg, char_emb, wjint);
                                    fw_char_lstm_root.add_input(cj);
                                }
                                fw_char_lstm_root.add_input(word_end);
		                Expression fw_r=fw_char_lstm_root.back();
                                //now backwards for root
                                bw_char_lstm_root.start_new_sequence();
             
                                bw_char_lstm_root.add_input(word_end);

                                while(!strevbuffer.empty()) {
		                    int wjint=strevbuffer.back();
	
	                       	    Expression cj=lookup(*hg, char_emb, wjint);
                                    bw_char_lstm_root.add_input(cj);
		                    strevbuffer.pop_back();
	                        }
                                 bw_char_lstm_root.add_input(word_start);    

                                 Expression bw_r=bw_char_lstm_root.back();

                                 
                                //backwards root done
                                 
				//cerr << "root of word done" << endl;
				//stem of a word
				fw_char_lstm_stem.start_new_sequence();
				fw_char_lstm_stem.add_input(word_start);
                                
				std::vector<int> strevbuffer_stem;
				for (unsigned j=0;j<ww_stem.length();j+=UTF8Len(ww_stem[j])){
                                     //    std::cout << j << " " << ww_stem << " stem char len: " << ww_stem.length() <<" " << ww_stem[j] << UTF8Len(ww_stem[j]) << endl;
					std::string wj;
                                    for (unsigned h=j;h<j+UTF8Len(ww_stem[j]);h++) wj+=ww_stem[h];
               
                                    int wjint=corpus.charsToInt[wj];
	                            strevbuffer_stem.push_back(wjint);
                                    Expression cj=lookup(*hg, char_emb, wjint);
                                    fw_char_lstm_stem.add_input(cj);
                                }
                
                                fw_char_lstm_stem.add_input(word_end);
                
				Expression fw_s=fw_char_lstm_stem.back();

                                 //now backwards for stem
                                bw_char_lstm_stem.start_new_sequence();
             
                                bw_char_lstm_stem.add_input(word_end);

                                while(!strevbuffer_stem.empty()) {
		                    int wjint=strevbuffer_stem.back();
	
	                       	    Expression cj=lookup(*hg, char_emb, wjint);
                                    fw_char_lstm_stem.add_input(cj);
		                    strevbuffer_stem.pop_back();
	                        }
                                bw_char_lstm_stem.add_input(word_start);    

                                Expression bw_s=bw_char_lstm_stem.back();
                                 
                                //backwards root done
                                 
                              
 			        //cerr << "stem of word done" << endl;
       
				vector<Expression> tt = {fw_r, bw_r, fw_s, bw_s};
...
//mycode begins
    std::vector<vector<WordLemmaPair>> train_lemmaSents;
    train_lemmaSents = generateLemmaSents("word-lemma-tr-ud-train.txt");
//my code ends
...
//mycode ends
	   ...
	   const vector<unsigned>& actions=corpus.correct_act_sent[order[si]];
           ComputationGraph hg;
           parser.log_prob_parser(&hg,sentence,tsentence,lemmaSent,sentencePos,actions,corpus.actions,corpus.intToWords,&right);
      ...
      if (logc % 25 == 1) { // report on dev set
        ...
        dev_lemmaSents = generateLemmaSents("word-lemma-tr-ud-dev.txt");
        //my code ends
        // dev_size = 100;
        double llh = 0;
        double trs = 0;
...
//mycode ends
	   ...
	   if (!USE_SPELLING) {
           ...
           ComputationGraph hg;
	   vector<unsigned> pred = parser.log_prob_parser(&hg,sentence,tsentence,lemmaSentDev,sentencePos,vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
	   double lp = 0;
...
//mycode begins
    ...
    std::vector<vector<WordLemmaPair>> test_lemmaSents;
    if (conf.count("train"))
       test_lemmaSents = generateLemmaSents("word-lemma-tr-ud-dev.txt");
    else
       test_lemmaSents = generateLemmaSents("word-lemma-tr-ud-test.txt");
//my code ends
 ...
 //mycode begins
      const vector<WordLemmaPair> lemmaSentTest = test_lemmaSents.at(sii);           
//mycode ends
      ...
      if (beam_size == 1)
        pred = parser.log_prob_parser(&cg,sentence,tsentence,lemmaSentTest,sentencePos,vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
      else
